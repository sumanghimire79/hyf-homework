...
// 1. short movie-title 
console.log('1. short movie-title ');
const shortMovieTitle = movies.filter(movie => movie.title.length<3);
console.log(shortMovieTitle);

// 2. Long movie-title
console.log('2. Long movie-title');

const longMovieTitle = movies.filter(movie => movie.title.length>56);
console.log(longMovieTitle);

// 3. number of movies made between 1980-1989 (including both the years)
console.log('3. movies between 1980-1989');

const movies1980to1989 = movies.filter(movie => movie.year>=1980 && movie.year<=1989 ).length;
console.log('The number of movies between 1980-1989 is: '+ movies1980to1989);

/* 
// 4. movies based on tags
const goodMovies = document.querySelector('#good-movies');
const titleGood = document.createElement('h2');
titleGood.innerHTML = 'Good Movies';
goodMovies.appendChild(titleGood);
const olGood = document.createElement('ol');

const averageMovies = document.querySelector('#average-movies');
const titleAverage = document.createElement('h2');
titleAverage.innerHTML = 'Average Movies';
averageMovies.appendChild(titleAverage);
const olAverage = document.createElement('ol');

const badMovies = document.querySelector('#bad-movies');
const titleBad = document.createElement('h2');
titleBad.innerHTML = 'Bad Movies';
badMovies.appendChild(titleBad);
const olBad = document.createElement('ol');

const moviesBasedonTags = movies.forEach(movie => 
  {
    let goodName;
    let averageName;
    let badName;
    if (movie.rating >= 7) {
      goodName = movie.title;
    } else if (movie.rating >= 4){
       averageName = movie.title;
    } else if (movie.rating < 4){
      badName =  movie.title;
    }
const liGood = document.createElement('li');
liGood.innerHTML = goodName ;
olGood.appendChild(liGood);
goodMovies.appendChild(olGood);

const liAverage = document.createElement('li');
liAverage.innerHTML = averageName ;
olAverage.appendChild(liAverage);
averageMovies.appendChild(olAverage);

const liBad = document.createElement('li');
liBad.innerHTML = badName ;
olBad.appendChild(liBad);
badMovies.appendChild(olBad);
  });

console.log('moviesBasedonTags');
console.log(moviesBasedonTags);

 */

// 4 movies based on tags --- 4 in anothr way
console.log('4. movies based on tags');

const moviesBasedonTags1 = movies.map(movie => 
  {
    ...
    // let cinema = Object.assign(movie, tagObj);

    return cinema;
  });
  console.log(moviesBasedonTags1);
 
  

  
  // 5. moviesRatedhigherSIX then only the rating 
  console.log('5. movies rated higher than 6');

  const moviesRatedhigherSIX = movies.filter(movie => movie.rating>6).map(movie => movie.rating);
  console.log(moviesRatedhigherSIX);
  
 
  // 6. the total number of movies containing any of following keywords: Surfer, Alien or Benjamin

 
  // suffer alien benjamin   .length
  console.log('6. filter for all : suffer alien benjamin');

const movieswithKeyWords =  movies.filter(movie => 
  movie.title ? movie.title.includes('Surfer') || movie.title.includes('Alien')|| movie.title.includes('Benjamin') : true );
console.log(movieswithKeyWords);

//only suffer
console.log('6.1 filter only suffer');

const movieswithKeyWords1 =  movies.filter(movie => 
movie.title ? movie.title.includes('Surfer') : true );
console.log(movieswithKeyWords1);

  //only alien
  console.log('6.2 filter only alien');

const movieswithKeyWords2 =  movies.filter(movie => 
movie.title ?  movie.title.includes('Alien') : true );
console.log(movieswithKeyWords2);
 
// only benjamin
console.log('6.3 filter only benjamin');

const movieswithKeyWords3 =  movies.filter(movie => 
movie.title ? movie.title.includes('Benjamin') : true );
console.log(movieswithKeyWords3);


// 7 repeat words
console.log('7. repeat words in movie title'); 

const repeatWordTitleMovies = movies.filter(movie => {
  const splitTitle = movie.title.toLowerCase().split(' ');
  const duplicates = splitTitle.filter((word, index) => {
    if (index !== splitTitle.indexOf(word)) {
         movie.title;
...
// console.log(repeatWordTitleMovies); // why empty array?     


// 8. Calculate the average rating of all the movies using reduce. Optional
console.log('8. Calculate the average rating of all the movies');

const averageRrating = movies.map(movie => movie.rating).reduce((totalRating, rating) => (totalRating+rating))/movies.length;
console.log('Average rating of movie is: '+ Math.floor(averageRrating*100)/100);

 
// 9. Count the total number of Good, Average and Bad movies using reduce
    // taking the array from Q4 moviesBasedonTags1 
    console.log('9. Total number of Good, Average and Bad movies ');

const ratings = moviesBasedonTags1.map((movie) => movie.tag).reduce((AllmovieTag, movieTag) => {
  if (movieTag in AllmovieTag) {
    AllmovieTag[movieTag]++;
  } else {
    AllmovieTag[movieTag] = 1;
  }
  return AllmovieTag;
}, []);
